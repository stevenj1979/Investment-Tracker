...
function getBittrexRequests($userID = 0){
  ...
  ,`BuyBackMinsToCancel`,`TimeToCancelBa`,`TimeStampNow`,`TimeStampTimeToCancel`,`BuyBackCounter`,`BuyBackMax`,`DisableBuyBack`
  ...
  while ($row = mysqli_fetch_assoc($result)){
    $tempAry[] = Array($row['Type'],	$row['BittrexRef'],	$row['ActionDate'],	$row['CompletionDate'],	$row['Status'],	$row['SellPrice'],	$row['UserName'],	$row['APIKey'],	$row['APISecret'],	$row['Symbol']//9
        ...
        ,$row['BuyBackMinsToCancel'],$row['TimeToCancelBa'],$row['TimeStampNow'],$row['TimeStampTimeToCancel'],$row['BuyBackCounter'],$row['BuyBackMax'],$row['DisableBuyBack']); //85
  }
  $conn->close();
  return $tempAry;
}
