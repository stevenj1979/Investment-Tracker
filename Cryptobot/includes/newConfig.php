...
function getBittrexRequests($userID = 0){
  ...
  ,getBTCPrice(84) as BTCPrice,getBTCPrice(85) as ETHPrice,`MultiSellRuleEnabled`,`MultiSellRuleTemplateID`,`StopBuyBack`,`MultiSellRuleID`,`TypeBa`,`ReduceLossBuy`,`IDBa`,`BuyOrderCancelTimeMins`,`MinsToCancelAction`,`MinsRemaining`
  ...
  while ($row = mysqli_fetch_assoc($result)){
    ...
    ,$row['MultiSellRuleTemplateID'],$row['StopBuyBack'],$row['MultiSellRuleID'],$row['TypeBa'],$row['ReduceLossBuy'],$row['IDBa'],$row['BuyOrderCancelTimeMins'],$row['MinsToCancelAction'],$row['MinsRemaining']); //58
  }
  $conn->close();
  return $tempAry;
}
...
function getTrackingCoins($whereclause, $table){
  ...
  if ($conn->connect_error) {
    ...
    ,`1HrPriceChangeLast`,`1HrPriceChange3`,`1HrPriceChange4`,`SecondstoUpdate`,`LastUpdated`,`Name`,`Image`,`DoNotBuy`,`HoursFlatPdcs`,`MinPriceFromLow`,`PctFromLiveToLow`,Trim(`6MonthPrice`)+0 as 6MonthPrice ,Trim(`3MonthPrice`)+0 as 3MonthPrice,Trim(`AverageLowPrice`)+0 as AverageLowPrice
    $table $whereclause ";
    //echo "<BR> $sql";
  $result = $conn->query($sql);
  //$result = mysqli_query($link4, $query);
...
function getTrackingSellCoinsAll(){
  ...
  ,`CaptureTrend`,`minsToDelay`,`Enabled` as `ReduceLossEnabled`,`SellPct` as `ReduceLossSellPct`,`OriginalPriceMultiplier`,`ReduceLossCounter`,`ReduceLossMaxCounter`,`HoursFlatLowPdcs` as `HoursFlat`,`OverrideReduceLoss`
 FROM `View5_SellCoins`  WHERE `Status` = 'Open' order by ProfitPct Asc ";
  ...
  while ($row = mysqli_fetch_assoc($result)){
    ...
    ,$row['ReduceLossCounter'],$row['ReduceLossMaxCounter'],$row['HoursFlat'],$row['OverrideReduceLoss']); //67
  }
  $conn->close();
  return $tempAry;
}
...
function WriteBuyBack($transactionID, $profitPct, $noOfRisesInPrice, $minsToCancel,$finalPrice,$amount,$cost,$usd_Amount){
  $conn = getSQLConn(rand(1,3));
  // Check connection
  if ($conn->connect_error) {
      die("Connection failed: " . $conn->connect_error);
  ...
  $sql = "Call WriteBuyBack($transactionID,$noOfRisesInPrice,$profitPct,$minsToCancel,$finalPrice,$amount,$cost,$usd_Amount);";
