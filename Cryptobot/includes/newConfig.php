...
function bittrexActionBuyBack($coinID,$oldBuyBackTransID,$buyBack = 1){
  $conn = getSQLConn(rand(1,3));
    ...
    $sql = "UPDATE `BittrexAction` SET `BuyBack` = $buyBack, `oldBuyBackTransID` = $oldBuyBackTransID, `MultiSellRuleID` = (SELECT `MultiSellRuleTemplateID` FROM `Transaction` WHERE `ID` = $oldBuyBackTransID)
             where `CoinID` = $coinID and `Type` = 'BuyBack' order by `ID` desc limit 1 ";
    print_r($sql);
    if ($conn->query($sql) === TRUE) {
        echo "New record created successfully";
    } else {
...
function addTrackingCoin($coinID, $coinPrice, $userID, $baseCurrency, $sendEmail, $buyCoin, $quantity, $ruleIDBuy, $coinSellOffsetPct, $coinSellOffsetEnabled, $buyType, $minsToCancelBuy, $sellRuleFixed, $toMerge, $noOfPurchases, $risesInPrice, $type, $originalPrice,$spreadBetTransID,$spreadBetRuleID,$overrideCoinAlloc,$callName, $transID = 0){
  ...
  VALUES ($coinID,$coinPrice,$userID,'$baseCurrency', $sendEmail, $buyCoin, $quantity, $ruleIDBuy, $coinSellOffsetPct, $coinSellOffsetEnabled, $buyType, $minsToCancelBuy, $sellRuleFixed, 'Open', $toMerge, $noOfPurchases,$originalPrice, $risesInPrice * ((SELECT Count(`ID`) 
