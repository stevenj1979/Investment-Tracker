...
function getTrackingCoins($whereclause, $table){
  ...
  while ($row = mysqli_fetch_assoc($result)){
    ...
    ,$row['HoursSinceAdded'],$row['MaxHoursFlat']);//47
  }
  $conn->close();
  return $tempAry;
}
...
function getTrackingSellCoins($type, $userID = 0){
  ...
  ,floor(timestampdiff(second,`OrderDate`, now())/3600) as `HoursSinceBuy`, `SellPctCsp`,`MaxHoursFlat`
  ...
  while ($row = mysqli_fetch_assoc($result)){
    ...
    ,$row['MultiSellRuleEnabled'],$row['HoursSinceBuy'],$row['SellPctCsp'],$row['MaxHoursFlat']); //68
  }
  $conn->close();
  return $tempAry;
}
...
function getTrackingSellCoinsAll(){
  ...
  ,`SavingOverride`,`HoursFlatRls`, `SpreadBetTransactionID`,`CoinSwapDelayed`,`MaxHoursFlat`,`PctOfAuto`,`PctOfAutoBuyBack`,`PctOfAutoReduceLoss`,`HoursFlatAutoEnabled`
 FROM `View5_SellCoins`  WHERE `Status` = 'Open' order by ProfitPct Asc ";
  ...
  while ($row = mysqli_fetch_assoc($result)){
    ...
    ,$row['HoursFlatRls'],$row['SpreadBetTransactionID'],$row['CoinSwapDelayed'],$row['MaxHoursFlat'],$row['PctOfAuto'],$row['PctOfAutoBuyBack'],$row['PctOfAutoReduceLoss'],$row['HoursFlatAutoEnabled']); //80
  }
  $conn->close();
  return $tempAry;
}
...
function getUserSellRules(){
  ...
  while ($row = mysqli_fetch_assoc($result)){
    ...
    ,$row['CalculatedSellPctStart'],$row['CalculatedSellPctEnd'],$row['CalculatedSellPctDays'],$row['BypassTrackingSell'],$row['CalculatedSellPctReduction'],$row['PctOfAuto']); //67
  }
  $conn->close();
  return $tempAry;
}
