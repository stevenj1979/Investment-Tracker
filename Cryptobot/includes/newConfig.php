...
function getBittrexRequests($userID = 0){
  ...
  ,`MinsToPauseAfterPurchase`,`OriginalAmount`,`SaveResidualCoins`,`MinsSinceAction`,`TimetoCancelBuyMins`,`BuyBack`,`oldBuyBackTransID`,`ResidualAmount`,`MergeSavingWithPurchase`
  ...
  while ($row = mysqli_fetch_assoc($result)){
    ...
    ,$row['MergeSavingWithPurchase'] ); //42
  }
  $conn->close();
  return $tempAry;
}
...
function getTrackingCoins($whereclause){
  $tempAry = [];
  $conn = getSQLConn(rand(1,3));
  // Check connection
  if ($conn->connect_error) {
    ...
    ,`1HrPriceChangeLast`,`1HrPriceChange3`,`1HrPriceChange4`,`SecondstoUpdate`,`LastUpdated`,`Name`,`Image`,`DoNotBuy`,`HoursFlatPdcs`,`MinPriceFromLow`,`PctFromLiveToLow`
  ...
  while ($row = mysqli_fetch_assoc($result)){
    ...
    ,$row['1HrPriceChange4'],$row['SecondstoUpdate'],$row['LastUpdated'],$row['Name'],$row['Image'],$row['DoNotBuy'],$row['HoursFlatPdcs'],$row['MinPriceFromLow'],$row['PctFromLiveToLow']);//42
  }
  $conn->close();
  return $tempAry;
}
...
function getTrackingSellCoinsAll(){
  ...
  ,`CaptureTrend`,`minsToDelay`,`Enabled` as `ReduceLossEnabled`,`SellPct` as `ReduceLossSellPct`,`OriginalPriceMultiplier`,`ReduceLossCounter`,`ReduceLossMaxCounter`,`HoursFlatPdcs` as `HoursFlat`
 FROM `View5_SellCoins`  WHERE `Status` = 'Open' order by ProfitPct Asc ";
  ...
  while ($row = mysqli_fetch_assoc($result)){
    ...
    ,$row['ReduceLossCounter'],$row['ReduceLossMaxCounter'],$row['HoursFlat']); //66
  }
  $conn->close();
  return $tempAry;
}
...
function getUserSellRules(){
  ...
  ,`PctUnderMaxPrice`
    FROM `View14_UserSellRules` WHERE `SellCoin` = 1";
  $result = $conn->query($sql);
  //$result = mysqli_query($link4, $query);
  //mysqli_fetch_assoc($result);
  while ($row = mysqli_fetch_assoc($result)){
    ...
    ,$row['MergeCoinEnabled'],$row['CoinModeRule'],$row['PctFromHighSellPriceEnabled'],$row['NoOfHoursFlatEnabled'],$row['NoOfHoursFlat'],$row['PctUnderMaxPrice']); //58
  }
  $conn->close();
  return $tempAry;
}
...
//  }
  ...
  //if ($btcBuyAmount > $minTradeAmount) {
    ...
    if ($btcBuyAmount >= $newMinTradeAmount  && $BTCBalance >= $buyMin){
      ...
      addCoinPurchaseDelay($coinID,$userID,120);
...
function getNewUSDTAlloc($userID,$overrideFlag){
  ...
  if ($overrideFlag){
    ...
    $sql = "SELECT (`USDTAlloc`) AllocTotal
            , (SELECT sum(`CoinPrice`*`Amount`)
            FROM `View15_OpenTransactions` WHERE `UserID` = $userID and `StatusTr` in ('Open','Pending') and `BaseCurrency` = 'USDT') as OpenTotal
            FROM `View16_CoinAllocation` WHERE `UserID` = $userID";
  }else{
...
function getNewBTCAlloc($userID,$overrideFlag){
  ...
  if ($overrideFlag){
    //echo "<BR> Flag1: $lowFlag";
    $sql = "SELECT `BTCAlloc`*getBTCPrice(84) as AllocTotal
            , (SELECT sum(`CoinPrice`*`Amount`)*getBTCPrice(84)
            FROM `View15_OpenTransactions` WHERE `UserID` = $userID and `StatusTr` in ('Open','Pending') and `BaseCurrency` = 'BTC') as OpenTotal
            FROM `View16_CoinAllocation` WHERE `UserID` = $userID";
  }else{
    //echo "<BR> Flag2: $lowFlag";
    //$sql = "SELECT `BTCAlloc`*getBTCPrice() as AllocTotal
    $sql = "SELECT if(`LowMarketModeEnabled`>0,((`BTCAlloc`*getBTCPrice(84))/100)*(`PctOnLow`*`LowMarketModeEnabled`+1),
...
function getNewETHAlloc($userID,$overrideFlag){
  ...
  if ($overrideFlag){
    //echo "<BR> Flag1: $lowFlag";
    $sql = "SELECT `ETHAlloc`*getBTCPrice(85) as AllocTotal
            , (SELECT sum(`CoinPrice`*`Amount`)*getBTCPrice(85)
            FROM `View15_OpenTransactions` WHERE `UserID` = $userID and `StatusTr` in ('Open','Pending') and `BaseCurrency` = 'ETH') as OpenTotal
            FROM `View16_CoinAllocation` WHERE `UserID` = $userID";
  }else{
    //echo "<BR> Flag2: $lowFlag";
    //$sql = "SELECT `ETHAlloc`*getETHPrice() as AllocTotal
    $sql = "SELECT if(`LowMarketModeEnabled`>0,((`ETHAlloc`*getBTCPrice(85))/100)*(`PctOnLow`*`LowMarketModeEnabled`+1),
    ((`ETHAlloc`*getBTCPrice(85))/100)*(`PctOnLow`*if(`LowMarketModeEnabled`>1,`LowMarketModeEnabled`,1))) as AllocTotal
            , (SELECT sum(`CoinPrice`*`Amount`)*getBTCPrice(85)
            FROM `View15_OpenTransactions` WHERE `UserID` = $userID and `StatusTr` in ('Open','Pending') and `BaseCurrency` = 'ETH') as OpenTotal
            FROM `View16_CoinAllocation` WHERE `UserID` = $userID";
  }
...
//12
  ...
  , $row['SaveResidualCoins'], $row['OriginalAmount'], $row['TrackingType'], $row['OriginalSellPrice'], $row['Profit'], $row['ProfitPct'], $row['ReEnableBuyRuleEnabled'], $row['ReEnableBuyRule']);
  }
  $conn->close();
  return $tempAry;
...
function getBuyBackData(){
  ...
  $sql = "SELECT `IDBb`, `TransactionIDBb`, `Quantity`, `SellPriceBb`, `StatusBb`, `SpreadBetTransactionID`, `SpreadBetRuleID`, `CoinID`, `SellPrice` as `SellPriceBA`, `LiveCoinPrice`
            ...
            ,if (`OriginalAmount`=0,`Quantity`,`OriginalAmount`) as `OriginalAmount`,`HoursFlatPdcs`
            FROM `View9_BuyBack`
  ...
  while ($row = mysqli_fetch_assoc($result)){
      ...
      ,$row['OverrideCoinAllocation'],$row['AllBuyBackAsOverride'],$row['BTCPrice'],$row['ETHPrice'],$row['LiveCoinPrice'],$row['DelayMins'],$row['OriginalAmount'],$row['HoursFlatPdcs']);
  }
  $conn->close();
  return $tempAry;
}
