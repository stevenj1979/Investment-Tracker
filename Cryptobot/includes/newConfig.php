...
function getSpreadBetData(){
  ...
  , `D7BuyPrice`,`PctofSixMonthHighPrice`,`PctofAllTimeHighPrice`,`DisableUntil`,`UserID`,`BuyFallsinPrice`,`CalculatedMinsToCancel` FROM `SpreadBetCoinStatsView` ";
  //echo "<BR> $sql";
  $result = $conn->query($sql);
  //$result = mysqli_query($link4, $query);
  //mysqli_fetch_assoc($result);
  while ($row = mysqli_fetch_assoc($result)){
      ...
      , $row['Hr1BuyPrice'], $row['Hr24BuyPrice'], $row['D7BuyPrice'], $row['PctofSixMonthHighPrice'], $row['PctofAllTimeHighPrice'], $row['DisableUntil'], $row['UserID'], $row['BuyFallsinPrice'], $row['CalculatedMinsToCancel']);
  }
  $conn->close();
  return $tempAry;
}
