...
function getBittrexRequests($userID = 0){
  ...
  `RuleIDSell`,`LiveCoinPrice`,`TimetoCancelBuy`,`BuyOrderCancelTime`,`KEK`, `Live7DChange`,`CoinModeRule`,`OrderDate`,`PctToSave`,`SpreadBetRuleID`,`SpreadBetTransactionID`
  ...
  while ($row = mysqli_fetch_assoc($result)){
    ...
    ,$row['TimetoCancelBuy'],$row['BuyOrderCancelTime'],$row['KEK'],$row['Live7DChange'],$row['CoinModeRule'],$row['OrderDate'],$row['PctToSave'],$row['SpreadBetRuleID'],$row['SpreadBetTransactionID']);
  }
  $conn->close();
  return $tempAry;
}
...
//12
  ...
  $sql = "SELECT `CoinID`,`CoinPrice`,`TrackDate`,`Symbol`,`LiveCoinPrice`,`PriceDifference`,`PctDifference`,`UserID`,`BaseCurrency`,`SendEmail`,`BuyCoin`,`Quantity`,`RuleIDBuy`,`CoinSellOffsetPct`
    ...
    ,`NoOfBuyModeOverrides`,`CoinModeOverridePriceEnabled`,`CoinMode`,`Type`, `LastPrice`,`SBRuleID`,`SBTransID`
  ...
  while ($row = mysqli_fetch_assoc($result)){
    ...
    ,$row['CoinMode'],$row['Type'],$row['LastPrice'],$row['SBRuleID'],$row['SBTransID']);
  }
  $conn->close();
  return $tempAry;
...
//12
  ...
  ,TIMESTAMPDIFF(MINUTE, `TrackStartDate`, Now()) as MinsFromStart, `SellFallsInPrice`,`Type`,`BaseSellPrice`,`LastPrice`,`Amount`*`LiveSellPrice` as BTCBuyAmount
  ...
  while ($row = mysqli_fetch_assoc($result)){
    ...
    ,$row['TotalRisesInPriceSell'],$row['TrackStartDate'],$row['MinsFromStart'],$row['SellFallsInPrice'], $row['Type'], $row['BaseSellPrice'], $row['LastPrice'], $row['BTCBuyAmount']);
  }
  $conn->close();
  return $tempAry;
...
function trackingCoinReadyToBuy($livePrice, $mins, $type, $buyPrice, $TransactionID, $NoOfRisesInPrice, $pctProfit, $minsFromDate, $lastPrice, $totalRisesInPrice){
  ...
  if ($currentPrice > $swingPrice){ //OR ($currentPrice < $swingPrice)
    ...
    updateNoOfRisesInPrice($TransactionID, 0);
    setNewTrackingPrice($livePrice, $TransactionID, 'Buy');
    return False;
  }
...
function getBuyBackData(){
  ...
  , `OriginalSaleProfit`, `OriginalSaleProfitPct`, `ProfitMultiply`, `NoOfRaisesInPrice`, `BuyBackPct`,`MinsToCancel`,`BullBearStatus`
   FROM `BuyBackView`";
  echo "<BR> $sql";
  $result = $conn->query($sql);
  //$result = mysqli_query($link4, $query);
  ...
  ,$row['OriginalSaleProfit'],$row['OriginalSaleProfitPct'],$row['ProfitMultiply'],$row['NoOfRaisesInPrice'],$row['BuyBackPct'],$row['MinsToCancel'],$row['BullBearStatus']);
  }
  $conn->close();
  return $tempAry;
}
...
function getTotalProfitSpreadBetSell($spreadBetTransactionID){
  ...
  $sql = "SELECT sum(`OriginalPurchasePrice`) as OriginalPurchasePrice ,sum(`LiveTotalPrice`) as LiveTotalPrice,sum(`SaleTotalPrice`) as SaleTotalPrice
