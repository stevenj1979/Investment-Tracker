...
function getBittrexRequests($userID = 0){
  ...
  while ($row = mysqli_fetch_assoc($result)){
    $tempAry[] = Array($row['Type'],	$row['BittrexRef'],	$row['ActionDate'],	$row['CompletionDate'],	$row['Status'],	$row['SellPrice'],	$row['UserName'],	$row['APIKey'],	$row['APISecret'],	$row['Symbol']//9
        ...
        ,	$row['holdingAmount'],	$row['NoOfPurchases'],	$row['Hr1PriceMovePct'],	$row['PctToCancelBittrexAction'],	$row['PctFromSale'],	$row['LiveProfitPct'],	$row['OneTimeBuyRuleBr'],	$row['DateADD']); //69
  }
  $conn->close();
  return $tempAry;
}
...
function getTrackingSellCoins($type, $userID = 0){
  ...
  ,floor(timestampdiff(second,`OrderDate`, now())/3600) as `HoursSinceBuy`
  ...
  while ($row = mysqli_fetch_assoc($result)){
    ...
    ,$row['MultiSellRuleEnabled'],$row['HoursSinceBuy']); //66
  }
  $conn->close();
  return $tempAry;
}
...
function getUserSellRules(){
  ...
  ,`PctUnderMaxPrice`,`HoursPastBuyToSellEnabled`, `HoursPastBuyToSell`, `CalculatedSellPctEnabled`, `CalculatedSellPctStart`, `CalculatedSellPctEnd`, `CalculatedSellPctDays`,`BypassTrackingSell`
  ...
  while ($row = mysqli_fetch_assoc($result)){
    ...
    ,$row['CalculatedSellPctStart'],$row['CalculatedSellPctEnd'],$row['CalculatedSellPctDays'],$row['BypassTrackingSell']); //65
  }
  $conn->close();
  return $tempAry;
}
