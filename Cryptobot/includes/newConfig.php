...
FROM `View5_SellCoins` $whereclause order by `ProfitPct` Desc ";
  ...
  while ($row = mysqli_fetch_assoc($result)){
    ...
    $row['CoinPricePctChange'],$row['LastSellOrders'],$row['LiveSellOrders'],$row['SellOrdersPctChange'],$row['LastVolume'],$row['LiveVolume'],$row['VolumePctChange'],$row['Last1HrChange'],$row['v'],$row['Hr1ChangePctChange'],$row['Last24HrChange'],$row['Live24HrChange'] //31
...
function getBuyBackData(){
  ...
  $sql = "SELECT `IDBb`, `TransactionIDBb`, `Quantity`, `SellPriceBb`, `StatusBb`, `SpreadBetTransactionID`, `SpreadBetRuleID`, `CoinID`, `SellPrice` as `SellPriceBA`, `LiveCoinPrice`
            ...
            ,`Hr1ChangePctChange`,`Hr24ChangePctChange`,`D7ChangePctChange`
            FROM `View9_BuyBack`
  ...
  while ($row = mysqli_fetch_assoc($result)){
      ...
      ,$row['CoinPrice'],$row['SaveMode'],$row['CoinPriceBB'],$row['USDBuyBackAmount'],$row['Hr1ChangePctChange'],$row['Hr24ChangePctChange'],$row['D7ChangePctChange']);
  }
  $conn->close();
  return $tempAry;
}
