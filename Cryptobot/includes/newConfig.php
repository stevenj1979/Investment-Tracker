...
function getBittrexRequests($userID = 0){
  ...
  ,`BuyBackMinsToCancel`,`TimeToCancelBa`,`TimeStampNow`,`TimeStampTimeToCancel`,`BuyBackCounter`,`BuyBackMax`,`DisableBuyBack`,`LiveCoinPriceSell`
  ...
  while ($row = mysqli_fetch_assoc($result)){
    $tempAry[] = Array($row['Type'],	$row['BittrexRef'],	$row['ActionDate'],	$row['CompletionDate'],	$row['Status'],	$row['SellPrice'],	$row['UserName'],	$row['APIKey'],	$row['APISecret'],	$row['Symbol']//9
        ...
        ,$row['BuyBackMinsToCancel'],$row['TimeToCancelBa'],$row['TimeStampNow'],$row['TimeStampTimeToCancel'],$row['BuyBackCounter'],$row['BuyBackMax'],$row['DisableBuyBack'],$row['LiveCoinPriceSell']); //86
  }
  $conn->close();
  return $tempAry;
}
...
function getPriceDipRules(){
  ...
  $sql = "SELECT `RuleID`,`EnableRuleActivationAfterDipBr`,`PriceDipEnable24Hour`,  `Hr24ChangePctChangeMkt` as `Hr24ChangePctChange`
          ...
          ,`PctOfAuto`,`DisableAfterDipPct`,`LiveCoinPriceMkt`,`marketHoursFlatLow`,`marketHoursFlatTarget`
  ...
  while ($row = mysqli_fetch_assoc($result)){
      ...
      ,$row['LiveCoinPriceMkt'],$row['marketHoursFlatLow'],$row['marketHoursFlatTarget']); //30
  }
  $conn->close();
  return $tempAry;
}
