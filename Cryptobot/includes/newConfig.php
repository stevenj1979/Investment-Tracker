...
function getTrackingCoins($whereclause, $table){
  ...
  if ($conn->connect_error) {
    ...
    ,`MaxHoursFlat`,`CaaOffset`,`CaaMinsToCancelBuy`
    $table $whereclause ";
    //echo "<BR> $sql <BR>";
  $result = $conn->query($sql);
  //$result = mysqli_query($link4, $query);
  ...
  while ($row = mysqli_fetch_assoc($result)){
    ...
    ,$row['HoursSinceAdded'],$row['MaxHoursFlat'],$row['CaaOffset'],$row['CaaMinsToCancelBuy']);//49
  }
  $conn->close();
  return $tempAry;
}
...
function getTrackingSellCoins($type, $userID = 0){
  ...
  ,floor(timestampdiff(second,`OrderDate`, now())/3600) as `HoursSinceBuy`, `SellPctCsp`,`MaxHoursFlat`,`Hr1Top`,`Hr1Bottom`,`CaaOffset`,`CaaMinsToCancelSell`
  ...
  while ($row = mysqli_fetch_assoc($result)){
    ...
    ,$row['MultiSellRuleEnabled'],$row['HoursSinceBuy'],$row['SellPctCsp'],$row['MaxHoursFlat'],$row['Hr1Top'],$row['Hr1Bottom'],$row['CaaOffset'],$row['CaaMinsToCancelSell']); //72
  }
  $conn->close();
  return $tempAry;
}
...
function getUserSellRules(){
  ...
  ,`PctOfAuto`,`OverrideBuyBackAmount`, `OverrideBuyBackSaving`,`HoursAfterPurchaseToStart`,`HoursAfterPurchaseToEnd`
  ...
  while ($row = mysqli_fetch_assoc($result)){
    ...
    ,$row['HoursAfterPurchaseToStart'],$row['HoursAfterPurchaseToEnd']); //71
  }
  $conn->close();
  return $tempAry;
}
...
function getBuyBackData(){
  ...
  $sql = "SELECT `IDBb`, `TransactionIDBb`, `Quantity`, `SellPriceBb`, `StatusBb`, `SpreadBetTransactionID`, `SpreadBetRuleID`, `CoinID`, `SellPrice` as `SellPriceBA`, `LiveCoinPrice`
            ...
            ,`PctOfAuto`,`BuyBackHoursFlatAutoEnabled`,`MaxHoursFlat`,`PctOfAutoReduceLoss`,`PctOfAutoBuyBack`,`bbMinsToCancel`,`BuyBackMinsToCancel`
            FROM `View9_BuyBack`
