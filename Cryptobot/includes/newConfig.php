...
function getTrackingSellCoins($type, $userID = 0){
  ...
  ,floor(timestampdiff(second,`OrderDate`, now())/3600) as `HoursSinceBuy`,`BypassTrackingSell`
  ...
  while ($row = mysqli_fetch_assoc($result)){
    ...
    ,$row['MultiSellRuleEnabled'],$row['HoursSinceBuy'],$row['BypassTrackingSell']); //67
  }
  $conn->close();
  return $tempAry;
}
...
function getUserSellRules(){
  ...
  ,`PctUnderMaxPrice`,`HoursPastBuyToSellEnabled`, `HoursPastBuyToSell`, `CalculatedSellPctEnabled`, `CalculatedSellPctStart`, `CalculatedSellPctEnd`, `CalculatedSellPctDays`
  ...
  while ($row = mysqli_fetch_assoc($result)){
    ...
    ,$row['CalculatedSellPctStart'],$row['CalculatedSellPctEnd'],$row['CalculatedSellPctDays']); //64
  }
  $conn->close();
  return $tempAry;
}
