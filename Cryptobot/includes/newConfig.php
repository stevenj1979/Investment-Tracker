...
function bittrexBuyComplete($bittrexRef,$transactionID, $finalPrice){
  ...
  newLogToSQL("bittrexBuyComplete","$sql",3,sQLUpdateLog,"SQL CALL","BittrexRef:$bittrexRef TransactionID:$transactionID");
}
...
//12
  ...
  $sql = "SELECT `CoinID`,`CoinPrice`,`TrackDate`,`Symbol`,`LiveCoinPrice`,`PriceDifference`,`PctDifference`,`UserID`,`BaseCurrency`,`SendEmail`,`BuyCoin`,`Quantity`,`RuleIDBuy`,`CoinSellOffsetPct`
    ...
    ,`OverrideCoinAllocation`,`OneTimeBuyRule`
  ...
  while ($row = mysqli_fetch_assoc($result)){
    ...
    ,$row['CoinMode'],$row['Type'],$row['LastPrice'],$row['SBRuleID'],$row['SBTransID'],$row['TrackingID'],$row['quickBuyCount'],$row['MinsDisabled'],$row['OverrideCoinAllocation'],$row['OneTimeBuyRule']);
  }
  $conn->close();
  return $tempAry;
...
//12
  ...
  , $row['SaveResidualCoins'], $row['OriginalAmount']);
  }
  $conn->close();
  return $tempAry;
...
function getSpreadCoinSellData($ID = 0){
  ...
  ,`PctToSave`,`CalculatedRisesInPrice`,`SpreadBetRuleID`,`PctProfitSell`,`AutoBuyBackSell`,`BounceTopPrice`,`BounceLowPrice`,`BounceDifference`,`DelayCoinSwap`
  ...
  while ($row = mysqli_fetch_assoc($result)){
    ...
    ,$row['SpreadBetRuleID'],$row['PctProfitSell'],$row['AutoBuyBackSell'],$row['BounceTopPrice'],$row['BounceLowPrice'],$row['BounceDifference'],$row['DelayCoinSwap']);
  }
  $conn->close();
  return $tempAry;
}
...
function trackingCoinReadyToSell($livePrice, $mins, $type, $basePrice, $TransactionID, $totalRisesInPrice, $pctProfit, $minsFromDate, $lastPrice, $NoOfRisesInPrice, $trackingSellID,$market1HrChangePct){
    ...
    echo "<BR>trackingCoinReadyToSell_OPT2: $mins | $minsFromDate | $livePrice | $basePrice | $NoOfRisesInPrice | $totalRisesInPrice | $trackingSellID | $TransactionID";
...
function getBuyBackData(){
  ...
  while ($row = mysqli_fetch_assoc($result)){
      ...
      ,$row['OverrideCoinAllocation'],$row['AllBuyBackAsOverride']);
  }
  $conn->close();
  return $tempAry;
}
...
function WriteBuyBack($transactionID, $profitPct, $noOfRisesInPrice, $minsToCancel){
  ...
  newLogToSQL("WriteBuyBack","$sql",3,sQLUpdateLog,"SQL CALL","TransactionID:$transactionID");
}
