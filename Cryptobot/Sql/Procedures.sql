...
ELSE
UPDATE `BittrexAction` SET `CoinID` = Coin_ID, `UserID` = User_ID , `Type` = n_Type, `BittrexRef` = Bittrex_Ref, `Status` = n_Status, `SellPrice` = Bit_Price, `RuleID` = Rule_ID, `TimeToCancel` = newDate, `ActionDate` = now() WHERE `TransactionID` = Trans_ID and `Type` = n_Type;
end if;
