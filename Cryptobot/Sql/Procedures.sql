...
DELIMITER $$
CREATE DEFINER=`stevenj1979`@`localhost` PROCEDURE `newerLogToSQL`(IN `User_ID` INT, IN `In_Sub` VARCHAR(100), IN `In_Comments` TEXT, IN `save_Total` INT, IN `Sub_Title` VARCHAR(50), IN `nRef` VARCHAR(50), IN `Days_To_Keep` INT)
    MODIFIES SQL DATA
BEGIN
  DECLARE CountOfAction INTEGER;
  ...
  END IF;
INSERT INTO `ActionLog`(`UserID`, `Subject`, `Comment`,`SubTitle`, `Reference`,`DateToDelete`) VALUES (User_ID,In_Sub,In_Comments, Sub_Title, nRef,date_add(now(),INTERVAL Days_To_Keep DAY));
...
INSERT INTO `CoinBuyHistory`(`ID`, `LiveCoinPrice`,`Symbol`,`BaseCurrency`,`ActionDate`,`LastCoinPrice`) VALUES (Coin_ID,bit_price,nSymbol ,base_curr,now(),last_Live);
...
SELECT ifnull(`MaxCoinPricePctChange`,0) into CoinPct FROM `MarketCoinStatsBaseCurr` where `BaseCurrency` = 'All';
SELECT ifnull(`MaxHr1ChangePctChange`,0) into Hr1Pct FROM `MarketCoinStatsBaseCurr` where `BaseCurrency` = 'All';
SELECT ifnull(`MaxHr24ChangePctChange`,0) into Hr24Pct FROM `MarketCoinStatsBaseCurr` where `BaseCurrency` = 'All';
SELECT ifnull(`MaxD7ChangePctChange`,0) into D7Pct FROM `MarketCoinStatsBaseCurr` where `BaseCurrency` = 'All';
SELECT ifnull(`MinCoinPricePctChange`,0) into MinCoinPct FROM `MarketCoinStatsBaseCurr` where `BaseCurrency` = 'All';
SELECT ifnull(`MinHr1ChangePctChange`,0) into MinHr1Pct FROM `MarketCoinStatsBaseCurr` where `BaseCurrency` = 'All';
SELECT ifnull(`MinHr24ChangePctChange`,0) into MinHr24Pct FROM `MarketCoinStatsBaseCurr` where `BaseCurrency` = 'All';
SELECT ifnull(`MinD7ChangePctChange`,0) into MinD7Pct FROM `MarketCoinStatsBaseCurr` where `BaseCurrency` = 'All';

Delete from `MarketCoinStatsBaseCurr` where `BaseCurrency` = 'All';

INSERT INTO `MarketCoinStatsBaseCurr`(`LiveCoinPrice`, `LastCoinPrice`, `Price3`, `Price4`, `Price5`, `CoinPricePctChange`, `LiveMarketCap`, `LastMarketCap`, `MarketCapPctChange`, `LiveBuyOrders`, `LastBuyOrders`, `BuyOrdersPctChange`, `LiveVolume`, `LastVolume`, `VolumePctChange`, `Live1HrChange`, `Live24HrChange`,  `Live7DChange`, `Hr1ChangePctChange`, `Hr24ChangePctChange`, `D7ChangePctChange`, `LiveSellOrders`, `LastSellOrders`, `SellOrdersPctChange`, `LivePriceTrend`, `LastPriceTrend`, `Price3Trend`, `Price4Trend`,`MaxCoinPricePctChange`,`MaxHr1ChangePctChange`,`MaxHr24ChangePctChange`,`MaxD7ChangePctChange`,`MinCoinPricePctChange`,`MinHr1ChangePctChange`,`MinHr24ChangePctChange`,`MinD7ChangePctChange`,`HoursFlat`,`HoursFlatLow`,`HoursFlatHigh`,`BaseCurrency`)
SELECT sum(LiveCoinPrice) as LiveCoinPrice, sum(LastCoinPrice) as LastCoinPrice, sum(Price3) as Price3, sum(Price4) as Price4, sum(Price5) as Price5
    ,sum(((LiveCoinPrice-`LastCoinPrice`)/`LastCoinPrice`)*100) as `CoinPricePctChange`
    , sum(`LiveMarketCap`) as LiveMarketCap, sum(`LastMarketCap`) as LastMarketCap, sum(((`LiveMarketCap`-`LastMarketCap`)/`LastMarketCap`)*100) as `MarketCapPctChange`
    ,  sum(`LiveBuyOrders`) as LiveBuyOrders, sum(`LastBuyOrders`) as LastBuyOrders, sum(((`LiveBuyOrders`-`LastBuyOrders`)/`LastBuyOrders`)* 100) as `BuyOrdersPctChange`
    , sum(`LiveVolume`) as LiveVolume, sum(`LastVolume`) as LastVolume, sum((( `LiveVolume`- `LastVolume`)/ `LastVolume`)*100) as `VolumePctChange`
    , sum(`Live1HrChange`) as Live1HrChange, sum(`Live24HrChange`) as Live24HrChange, sum(`Live7DChange`) as Live7DChange
    ,(sum(LiveCoinPrice-`Live1HrChange`)/ `Live1HrChange`)*100 as `Hr1ChangePctChange`
    ,(sum(LiveCoinPrice-`Live24HrChange`)/ `Live24HrChange`)*100   as `Hr24ChangePctChange`
    , (sum(LiveCoinPrice-`Live7DChange`)/ `Live7DChange`)*100   as `D7ChangePctChange`
    , sum(`LiveSellOrders`) as LiveSellOrders,sum(`LastSellOrders`) as LastSellOrders,sum(((`LiveSellOrders`-`LastSellOrders`)/`LastSellOrders`)*100) as `SellOrdersPctChange`
    ...
    ,if(sum(((`LiveCoinPrice`-`LastCoinPrice`)/`LastCoinPrice`)*100) > CoinPct, sum(((`LiveCoinPrice`-`LastCoinPrice`)/`LastCoinPrice`)*100), CoinPct)
    ,if(  ((sum(`LiveCoinPrice`) - sum(`Hr1ChangePctChange`))/sum(`LiveCoinPrice`))*100 > Hr1Pct, if(((sum(`LiveCoinPrice`) - sum(`Hr1ChangePctChange`))/sum(`LiveCoinPrice`))*100 < 99,((sum(`LiveCoinPrice`) - sum(`Hr1ChangePctChange`))/sum(`LiveCoinPrice`))*100, Hr1Pct), Hr1Pct)
    ,if( (( sum(`LiveCoinPrice`)- sum(`Hr24ChangePctChange`))/sum(`LiveCoinPrice`))*100 > Hr24Pct, if( (( sum(`LiveCoinPrice`)- sum(`Hr24ChangePctChange`))/sum(`LiveCoinPrice`))*100 < 99,  (( sum(`LiveCoinPrice`)- sum(`Hr24ChangePctChange`))/sum(`LiveCoinPrice`))*100, Hr24Pct), Hr24Pct)
    ,if( ((sum(`LiveCoinPrice`) - sum(`D7ChangePctChange`))/sum(`LiveCoinPrice`))*100 > D7Pct, if( ((sum(`LiveCoinPrice`) - sum(`D7ChangePctChange`))/sum(`LiveCoinPrice`))*100 < 99, ((sum(`LiveCoinPrice`) - sum(`D7ChangePctChange`))/sum(`LiveCoinPrice`))*100 , D7Pct) , D7Pct)
    ,if(sum(((`LiveCoinPrice`-`LastCoinPrice`)/`LastCoinPrice`)*100) < MinCoinPct, sum(((`LiveCoinPrice`-`LastCoinPrice`)/`LastCoinPrice`)*100), MinCoinPct)
    ,if(  ((sum(`LiveCoinPrice`) - sum(`Hr1ChangePctChange`))/sum(`LiveCoinPrice`))*100 < MinHr1Pct, if(  ((sum(`LiveCoinPrice`) - sum(`Hr1ChangePctChange`))/sum(`LiveCoinPrice`))*100 > -99,  ((sum(`LiveCoinPrice`) - sum(`Hr1ChangePctChange`))/sum(`LiveCoinPrice`))*100, MinHr1Pct), MinHr1Pct)
    ...
    ,avg(`HoursFlat`) as HoursFlat,avg(`HoursFlatLow`) as HoursFlatLow,avg(`HoursFlatHigh`) as HoursFlatHigh, 'ALL'
...
INSERT INTO `HistoricBittrexBalances`(`Symbol`, `Total`, `Price`, `UserID`, `Multiplier`,`TotalUSD`,`CoinID`,`BaseCurrency`,`DateDay`,`DateMonth`,`DateYear`)
        ...
        ,`Cn`.`ID`,`Bb`.`BaseCurrency`, Day(now()),Month(now()),Year(now())
...
DELIMITER $$
CREATE DEFINER=`stevenj1979`@`localhost` PROCEDURE `DisableRuleOnLowBalance`(IN `User_ID` INT)
    NO SQL
BEGIN
...
SELECT `Total`*getBTCPrice(84) into BTC_Bal FROM `BittrexBalances` WHERE `UserID` = User_ID and Day(`Date`) = Day(now()) and Month(`Date`) = Month(now()) and Year(`Date`) = Year(now()) and `Symbol` = 'BTC';

SELECT `Total`*getBTCPrice(83) into USDT_Bal FROM `BittrexBalances` WHERE `UserID` = User_ID and Day(`Date`) = Day(now()) and Month(`Date`) = Month(now()) and Year(`Date`) = Year(now()) and `Symbol` = 'USDT';
...
if (USDT_Bal < 15) THEN
    ...
    Update `Transaction` `Tr` join `Coin` `Cn` on `Cn`.`ID` = `Tr`.`CoinID` SET`LowBalanceDoNotBuy`= 0 WHERE `Tr`.`Status` = 'Open' and `Cn`.`BaseCurrency` = 'USDT' and `Tr`.`UserID` = User_ID;
end if;
if (ETH_Bal < 15) THEN
		UPDATE `BuyRules` SET `LowBalanceDoNotBuy`= 1 WHERE `LimitToBaseCurrency` = 'ETH' and `UserID` = User_ID;
        Update `Transaction` `Tr` join `Coin` `Cn` on `Cn`.`ID` = `Tr`.`CoinID` SET `LowBalanceDoNotBuy`= 1  WHERE `Tr`.`Status` = 'Open' and `Cn`.`BaseCurrency` = 'ETH' and `Tr`.`UserID` = User_ID;
else
	UPDATE `BuyRules` SET `LowBalanceDoNotBuy`= 0 WHERE `LimitToBaseCurrency` = 'ETH' and `UserID` = User_ID;
    Update `Transaction` `Tr` join `Coin` `Cn` on `Cn`.`ID` = `Tr`.`CoinID` SET `LowBalanceDoNotBuy`= 0  WHERE `Tr`.`Status` = 'Open' and `Cn`.`BaseCurrency` = 'ETH' and `Tr`.`UserID` = User_ID;
end if;
END$$
DELIMITER ;

DELIMITER $$
CREATE DEFINER=`stevenj1979`@`localhost` PROCEDURE `UpdateMarketStatsCurr`(IN `Base_Curr` VARCHAR(50))
    MODIFIES SQL DATA
BEGIN
DECLARE CoinPct DEC(20,14);
DECLARE Hr1Pct DEC(20,14);
...
SELECT ifnull(`MaxCoinPricePctChange`,0) into CoinPct FROM `MarketCoinStatsBaseCurr` where `BaseCurrency` = Base_Curr;
SELECT ifnull(`MaxHr1ChangePctChange`,0) into Hr1Pct FROM `MarketCoinStatsBaseCurr` where `BaseCurrency` = Base_Curr;
SELECT ifnull(`MaxHr24ChangePctChange`,0) into Hr24Pct FROM `MarketCoinStatsBaseCurr` where `BaseCurrency` = Base_Curr;
SELECT ifnull(`MaxD7ChangePctChange`,0) into D7Pct FROM `MarketCoinStatsBaseCurr` where `BaseCurrency` = Base_Curr;
SELECT ifnull(`MinCoinPricePctChange`,0) into MinCoinPct FROM `MarketCoinStatsBaseCurr` where `BaseCurrency` = Base_Curr;
SELECT ifnull(`MinHr1ChangePctChange`,0) into MinHr1Pct FROM `MarketCoinStatsBaseCurr` where `BaseCurrency` = Base_Curr;
SELECT ifnull(`MinHr24ChangePctChange`,0) into MinHr24Pct FROM `MarketCoinStatsBaseCurr` where `BaseCurrency` = Base_Curr;
SELECT ifnull(`MinD7ChangePctChange`,0) into MinD7Pct FROM `MarketCoinStatsBaseCurr` where `BaseCurrency` = Base_Curr;

Delete from `MarketCoinStatsBaseCurr` where `BaseCurrency` = Base_Curr;
