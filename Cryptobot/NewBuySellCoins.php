  ...
  for($a = 0; $a < $sellCoinsLength; $a++) {
    ...
    for($z = 0; $z < $sellRulesSize; $z++) {//Sell Rules
      ...
      if ($calculatedSellPctEnable == 1){
        ...
        $amountToReduce = ($sellPctCsp/100)*25;
        $ProfitPctBtm_Sell = $ProfitPctBtm_Sell - $amountToReduce;
        writeCalculatedSellPct($transactionID,$sellCoinsUserID,$ProfitPctBtm_Sell);
...
function runBittrex($BittrexReqs,$apiVersion){
  ...
  for($b = 0; $b < $BittrexReqsSize; $b++) {
    ...
    $overrideBittrexCancellation = $BittrexReqs[$b][71]; $currentTime = $BittrexReqs[$b][73]; $dateAdd = $BittrexReqs[$b][69];
    ...
    echo "<BR> CurrentTime:$finalCurrentTime | Cancel Time $finalTimeToCancel | $date | $timeToCancelMins | $dateAdd";
