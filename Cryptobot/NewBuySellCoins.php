...
function runBuyBack($buyBackCoins,$webSettingsAry){
  ...
  for ($t=0; $t<$buyBackCoinsSize;$t++){
    ...
    SuperLog($nFile,"<BR> Check Price: $bBID | $priceDifferecePct | $buyBackPct",$nFunc,"BB1","BBID:$bBID",$logVariSettingAry);
    if ($profitPct <=  $buyBackPct AND $delayCoinPurchase <> 1){
      //if($delayMins > 0){ echo "<B> EXIT: Delay:$delayMins"; continue; }
      SuperLog($nFile,"<BR> $priceDifferecePct <=  ($buyBackPct+$profitMultiply)",$nFunc,"BB2","BBID:$bBID",$logVariSettingAry);
      ...
      SuperLog($nFile,": $priceDifferecePct | BuyBackPct: $buyBackPct Bull/Bear: $bullBearStatus | SellPrice: $sellPriceBA | LivePrice: $liveCoinPrice | BBID: $bBID | LCP: $tmpLiveCoinPrice",$nFunc,"BB3_PriceDiffPct","BBID:$bBID",$logVariSettingAry);
      ...
      if ($coinAllocation <= $coinAllocMin && $allBuyBackAsOverride == 0){
          SuperLog($nFile,"<BR> EXIT CoinAllocation: $tmpBaseCur | $type | $BTCAmount | $ogBTCAmount| $coinAllocation",$nFunc,"BB4","BBID:$bBID",$logExitSettingAry);
      ...
      //}else{
        ...
        SuperLog($nFile,"Qty:$quantity CPBB: $coinPriceBB | $origPurchasePrice SPBA: $livePriceUSD | $livePriceUSD Profit: $profit PCT: $profitPct",$nFunc,"BB6","BBID:$bBID",$logVariSettingAry);
        SuperLog($nFile,"$buyBackPurchasePrice = $livePriceUSD + $bbKittyAmount; | $originalAmount * $livePriceUSD;| $saveMode | $profitPct",$nFunc,"BB7","BBID:$bBID",$logVariSettingAry);
      //}
      ...
      if ($hoursFlat<$hoursFlatTarget){ SuperLog($nFile,"HoursFlat: $hoursFlat",$nFunc,"BB8","BBID:$bBID",$logExitSettingAry);  continue;}
      ...
      if ($tmpLiveCoinPrice <> 0){
        if ($tmpSBRuleID <> 0){
          ...
          for ($p=0;$p<$spreadbetBuyBackSize;$p++){
            if ($spreadbetBuyBackSize > 3){
              ...
              SuperLog($nFile,"BuyBackKitty","Adding $bbKittyAmount to $bBID | TotalBTC: $BTC_BB_Amount| Total USDT: $usdt_BB_Amount| TotalETH: $eth_BB_Amount | BTC_P: $portionBTC| USDT_P: $portion| ETH_P: $portionETH",$nFunc,"BB10","BBID:$bBID",$logVariSettingAry);
              //CloseBuyBack
              closeBuyBack($bBID);
          ...
          SuperLog($nFile,"addTrackingCoin($tmpCoinID, $tmpLiveCoinPrice, $tmpUserID, $tmpBaseCur, $tmpSendEmail, $tmpBuyCoin, $buyBackPurchasePrice, $tmpBuyRule, $tmpOffset, $tmpOffsetEnabled, $tmpBuyType, 240, $tmpFixSellRule,$tmpToMerge,$tmpNoOfPurchases,$noOfRaisesInPrice,$tmpType,$tmpLiveCoinPrice,$tmpSBTransID,$tmpSBRuleID);",$nFunc,"BB11","BBID:$bBID",$logFlowSettingAry);
          SuperLog($nFile,"Adding $bbKittyAmount to $bBID | TotalBTC: $BTC_BB_Amount| Total USDT: $usdt_BB_Amount| TotalETH: $eth_BB_Amount | BTC_P: $portionBTC| USDT_P: $portion| ETH_P: $portionETH",$nFunc,"BB12","BBID:$bBID",$logVariSettingAry);
          //CloseBuyBack
          closeBuyBack($bBID);
          //addWebUsage($userID,"Remove","BuyBack");
          //addWebUsage($userID,"Add","BuyTracking");
        ...
        SuperLog($nFile,"LivePrice: $tmpLiveCoinPrice Coin:$tmpSymbol CoinID:$tmpCoinID PriceWBuffer:$tmpOriginalPriceWithBuffer",$nFunc,"BB13","BBID:$bBID",$logVariSettingAry);
      }
    }
  }
  return $finalBool;
...
function runBuyCoins($coins,$userProfit,$marketProfit,$ruleProfit,$totalBTCSpent,$dailyBTCSpent,$baseMultiplier,$delayCoinPurchase,$buyRules,$coinPriceMatch,$coinPricePatternList,$coin1HrPatternList,$autoBuyPrice,$trackCounter,$buyCounter,$ruleType,$webSettingsAry){
  ...
  $finalBool = False;
  echo "<blockquote>";
  SuperLog($nFile,"BuyCoin Key: ",$nFunc,"BC1","",$logFlowSettingAry);
  SuperLog($nFile,"1: MarketCap | 2: Volume | 3: BuyOrders | 4: 1HrPctChange | 5: 24HrPctChange  ",$nFunc,"BC2","",$logVariSettingAry);
  SuperLog($nFile,"6: 7DPctChange | 7: CoinPrice | 8: SellOrders | 9: PriceTrend | 10: MinPrice ",$nFunc,"BC3","",$logVariSettingAry);
  SuperLog($nFile,"11: AutoPrice | 12: CoinPattern | 13: GlobalAllDisabled | 14: 1HrPattern | 15: HoursFlat ",$nFunc,"BC4","",$logVariSettingAry);
  SuperLog($nFile,"16: PriceDipMin <BR>",$nFunc,"BC5","",$logVariSettingAry);
  $coinLength = Count($coins);
  $buyRulesSize = count($buyRules);
  SuperLog($nFile,"Coin Length: $coinLength  RuleLength: $buyRulesSize <BR>",$nFunc,"BC6","",$logVariSettingAry);
  for($x = 0; $x < $coinLength; $x++) {
    ...
    for($y = 0; $y < $buyRulesSize; $y++) {
      ...
      SuperLog($nFile, "This is a Test | $KEK | $APISecret | $APIKey",$nFunc,"BC8","",$logVariSettingAry);
      if (!Empty($KEK)){$APISecret = decrypt($KEK,$buyRules[$y][31]);}
      SuperLog($nFile, "FinalPriceDipMinPrice:  $finalPriceDipMinPrice = $priceDipMinPrice - (($priceDipMinPrice/100)*$pctOverMinPrice); LIVE: $LiveCoinPrice<BR>",$nFunc,"BC9","",$logVariSettingAry);
      ...
      SuperLog($nFile,"Placeholder 1:  $coinID - $symbol - $baseCurrency  RuleID: $ruleIDBuy | $ruleType <BR>",$nFunc,"BC10","",$logFlowSettingAry);
      ...
      SuperLog($nFile,"Placeholder 2:  $coinID - $symbol - $baseCurrency  RuleID: $ruleIDBuy <BR>",$nFunc,"BC11","",$logFlowSettingAry);
      ...
      for ($b=0; $b<$delayCoinPurchaseSize; $b++){
        ...
        if ($delayCoinPurchaseUserID == $userID AND $delayCoinPurchaseCoinID == $coinID){
          SuperLog($nFile,"EXIT: Delay CoinID: $coinID! ",$nFunc,"BC12","",$logExitSettingAry); continue 2;
        }
      }

      if ($priceDipMinPriceEnabled == 1){
        if ($hoursSinceAdded < 3000){
          SuperLog($nFile,"EXIT: PriceDip Hours Since Added less than 300: $hoursSinceAdded | $coinID - $symbol - $baseCurrency  RuleID: $ruleIDBuy <BR>",$nFunc,"BC13","",$logExitSettingAry);
          continue;
        }
      }
      SuperLog($nFile,"Placeholder 3:  $coinID - $symbol - $baseCurrency  RuleID: $ruleIDBuy <BR>",$nFunc,"BC14","",$logFlowSettingAry);
      $ruleProfitSize = count($ruleProfit);
      for ($h=0; $h<$ruleProfitSize; $h++){
          if ($limitBuyAmountEnabled == 1){
            SuperLog($nFile,	"TEST limitBuyAmountEnabled: $limitBuyAmountEnabled | ".$ruleProfit[$h][4]." | $ruleIDBuy | ".$ruleProfit[$h][1]." | $limitBuyAmount",$nFunc,"BC15","",$logVariSettingAry);
            ...
            if ($ruleProfit[$h][4] == $ruleIDBuy and $ruleProfit[$h][1] >= $limitBuyTransactions){SuperLog($nFile, "EXIT: Rule Amount Exceeded! ",$nFunc,"BC16","",$logExitSettingAry); continue;}
          }
          if ($limitBuyTransactionsEnabled == 1 and $coinModeOverridePriceEnabled == 0){
            SuperLog($nFile,"TEST limitBuyTransactionEnabled: $limitBuyTransactionsEnabled | ".$ruleProfit[$h][4]." | $ruleIDBuy | ".$ruleProfit[$h][5]." | $limitBuyTransactions",$nFunc,"BC17","",$logVariSettingAry);
            ...
            if ($ruleProfit[$h][4] == $ruleIDBuy and $ruleProfit[$h][5] >= $limitBuyTransactions){SuperLog($nFile,"EXIT: Rule Transaction Count Exceeded! ",$nFunc,"BC18","",$logExitSettingAry); continue;}
          }elseif($coinModeOverridePriceEnabled == 1 and $limitBuyAmountEnabled == 1){
            SuperLog($nFile,"TEST limitBuyTransactionEnabled: $limitBuyAmount | $noOfBuyModeOverrides | ".$ruleProfit[$h][5],$nFunc,"BC19","",$logVariSettingAry);
            ...
            if (($limitBuyAmount + $noOfBuyModeOverrides) >=  $ruleProfit[$h][5]){ SuperLog($nFile,"EXIT: Rule Transaction Count Override Exceeded! ",$nFunc,"BC20","",$logExitSettingAry); continue;}
          }
      }

      if (isset($marketProfit[0][0])){
        if ($MarketDropStopEnabled == 1 and $marketProfit[0][0] <= $marketDropStopPct and $overrideDisableRule == 0){
          SuperLog($nFile, "Market Profit Enbled: $MarketDropStopEnabled Pct: $marketDropStopPct current: ".$marketProfit[0][0]." | RuleID $ruleIDBuy",$nFunc,"BC21","RuleID:$ruleIDBuy CoinID:$coinID",$logVariSettingAry);
          ...
          SuperLog($nFile, "pauseRule($ruleIDBuy,0, $userID);| MarketProfit: ".$marketProfit[0][1],$nFunc,"BC22","RuleID:$ruleIDBuy CoinID:$coinID",$logVariSettingAry);
          pauseRule($ruleIDBuy,0, $userID);
        }
      }
      SuperLog($nFile, "Placeholder 4:  $coinID - $symbol - $baseCurrency  RuleID: $ruleIDBuy <BR>",$nFunc,"BC23","",$logFlowSettingAry);
      ...
      if ($totalProfitPauseEnabled == 1 && $profitNum<= $totalProfitPause && $ruleIDBuy == $rulesPause){
        if ($rulesPauseEnabled == 1){
          SuperLog($nFile, "PAUSING RULES $rulesPause for $rulesPauseHours HOURS",$nFunc,"BC24","",$logExitSettingAry);
        ...
        SuperLog($nFile, "EXIT: TotalProfitPauseEnabled $totalProfitPauseEnabled Profit: $profitNum $totalProfitPause ",$nFunc,"BC25","",$logExitSettingAry);
        continue;}
        //else{ echo "<BR> EXIT PROFIT!";}
      $GLOBALS['allDisabled'] = false;
      if (empty($APIKey) && empty($APISecret)){ SuperLog($nFile, "EXIT: No API Key  $coinID - $symbol - $baseCurrency  RuleID: $ruleIDBuy <BR>",$nFunc,"BC26","",$logExitSettingAry); continue;}
      ...
      if ($limitToCoin != "ALL" && $symbol != $limitToCoin) {
        ...
        SuperLog($nFile,"EXIT: Wrong Symbol for Rule  $coinID - $symbol - $baseCurrency  RuleID: $ruleIDBuy <BR>",$nFunc,"BC29","",$logExitSettingAry);
        continue;}
      SuperLog($nFile,"Placeholder 5:  $coinID - $symbol - $baseCurrency  RuleID: $ruleIDBuy <BR>",$nFunc,"BC30","",$logFlowSettingAry);
      if ($doNotBuy == 1){
        //Echo "<BR> EXIT: Do Not Buy<BR>";
        SuperLog($nFile,"EXIT: Do NOT Buy  $coinID - $symbol - $baseCurrency  RuleID: $ruleIDBuy <BR>",$nFunc,"BC31","",$logExitSettingAry);
        continue;}
      if ($overrideDailyLimit == 0 && $EnableTotalBTCLimit == 1){
        SuperLog($nFile,"Check if over total limit! ",$nFunc,"BC32","",$logFlowSettingAry);
        $userBTCSpent = getUserTotalBTC($totalBTCSpent,$userID,$baseCurrency);
        SuperLog($nFile,"Testing Testing Testing| $userID | : ".$totalBTCSpent[0][0],$nFunc,"BC33","",$logFlowSettingAry);
          if ($userBTCSpent >= $TotalBTCLimit){ SuperLog($nFile,"EXIT: TOTAL BTC SPENT",$nFunc,"BC34","",$logExitSettingAry); continue;}else{  SuperLog($nFile,"Total Spend ".$userBTCSpent." Limit $TotalBTCLimit",$nFunc,"BC35","",$logExitSettingAry);}
      }
      if ($overrideDailyLimit == 0 && $EnableDailyBTCLimit == 1){
        SuperLog($nFile,"Check if over daily limit! ",$nFunc,"BC36","",$logVariSettingAry);
        $userDailyBTCSpent = getUserTotalBTC($dailyBTCSpent,$userID,$baseCurrency);
          if ($userDailyBTCSpent >= $DailyBTCLimit){SuperLog($nFile,"EXIT: DAILY BTC SPENT",$nFunc,"BC37","",$logExitSettingAry);continue;}else{ SuperLog($nFile,"Daily Spend ".$userDailyBTCSpent." Limit $DailyBTCLimit",$nFunc,"BC38","",$logVariSettingAry);}
      }

      SuperLog($nFile,"Placeholder 6:  $coinID - $symbol - $baseCurrency  RuleID: $ruleIDBuy <BR>",$nFunc,"BC38","",$logVariSettingAry);
      if ($buyCounter[$userID."-".$coinID] >= 1 && $overrideDailyLimit == 0){ SuperLog($nFile,"EXIT: Buy Counter Met! $noOfBuys ".$buyCounter[$userID."-".$coinID],$nFunc,"BC39","",$logExitSettingAry);//continue;
      }
      if ($buyCounter[$userID."-Total"] >= $noOfBuys && $overrideDailyLimit == 0){ SuperLog($nFile,"EXIT: Buy Counter Met! $noOfBuys ".$buyCounter[$userID."-Total"],$nFunc,"BC40","",$logExitSettingAry);//continue;
      }
      if ($userActive == False){ SuperLog($nFile,"EXIT: User Not Active!",$nFunc,"BC41","",$logExitSettingAry); continue;}
      if ($hoursDisableUntil > 0){ SuperLog($nFile,"EXIT: Disabled until: ".$hoursDisableUntil,$nFunc,"BC42","",$logExitSettingAry); continue;}
      ...
      SuperLog($nFile,"TEST 10: $BuyPriceMinEnabled | $BuyPriceMin | $LiveCoinPrice | $test10",$nFunc,"BC43","",$logVariSettingAry);
      ...
      SuperLog($nFile,"checkPriceDipCoinFlat($priceDipCoinFlatEnabled,$priceDipHoursFlatTarget, $priceDipHours,$checkPriceDipCoinFlatArt);",$nFunc,"BC44","",$logVariSettingAry);
      ...
      SuperLog($nFile,"buyWithMin($priceDipMinPriceEnabled, $finalPriceDipMinPrice, $LiveCoinPrice,$BuyCoinBuyWithMin10,'PriceDipMinPrice16');",$nFunc,"BC45","",$logVariSettingAry);
      ...
      if (buyAmountOverride($buyAmountOverrideEnabled)){$BTCAmount = $buyAmountOverride; SuperLog($nFile,"13: BuyAmountOverride set to : $buyAmountOverride | BTCAmount: $BTCAmount",$nFunc,"BC46","",$logVariSettingAry);}
      ...
      if ($totalScore_Buy >= 15 ){
        ...
        SuperLog($nFile,"UserID: $userID | RuleID: $ruleIDBuy | Coin : $symbol| 1:$test1  2:$test2  3:$test3  4:$test4  5:$test5  6:$test6  7:$test7  8:$test8  9:$test9  10:$test10  11:$test11  12:$test12  13:$test13  14:$test14 15:$test15 16:$test16  TOTAL:$totalScore_Buy / 16",$nFunc,"BC47","",$logFlowSettingAry);
      }else{
        if ($showLowScores == 1){
            SuperLog($nFile,"UserID: $userID | RuleID: $ruleIDBuy | Coin : $symbol| 1:$test1  2:$test2  3:$test3  4:$test4  5:$test5  6:$test6  7:$test7  8:$test8  9:$test9  10:$test10  11:$test11  12:$test12  13:$test13  14:$test14 15:$test15 16:$test16  TOTAL:$totalScore_Buy / 16",$nFunc,"BC48","",$logFlowSettingAry);
        }
      }
      ...
      $test11 == True && $test12 == True && $test13 == True && $test14 == True && $test15 == True && $test16 == True){
        ...
        SuperLog($nFile,"Call Bittrex Bal: bittrexbalance($APIKey, $APISecret,$baseCurrency, $apiVersion);",$nFunc,"BC49","",$logFlowSettingAry);
        $BTCBalance = bittrexbalance($APIKey, $APISecret,$baseCurrency, $apiVersion);
        $reservedAmount = getReservedAmount($baseCurrency,$userID);
        SuperLog($nFile,"TEST BAL AND RES: $BTCBalance ; $BTCAmount ; ".$reservedAmount[0][0]."| ",$nFunc,"BC50","",$logVariSettingAry);//.$BTCBalance-$reservedAmount
        ...
        SuperLog($nFile,"$usdtReserved | $btcReserved | $ethReserved",$nFunc,"BC51","",$logFlowSettingAry);
        $totalReserved = $usdtReserved+$btcReserved+$ethReserved;

        if ($baseCurrency == 'BTC'){
          SuperLog($nFile,"BTC Bal Test : $BTCBalance | $totalReserved | ".$baseMultiplier[0][0],$nFunc,"BC52","",$logVariSettingAry);
          $totalBal = ($BTCBalance*$baseMultiplier[0][0])-$totalReserved;
          $buyQuantity = $BTCAmount / $baseMultiplier[0][0];
          SuperLog($nFile,"BaseCurrency is BTC : totalBal: $totalBal | BTC Bal: $BTCBalance | totalReserved: $totalReserved | Multiplier : ".$baseMultiplier[0][0],$nFunc,"BC53","RuleID:$ruleIDBuy CoinID:$coinID",$logVariSettingAry);
        }elseif ($baseCurrency == 'ETH'){
          SuperLog($nFile,"ETH Bal Test : $BTCBalance | $totalReserved | ".$baseMultiplier[0][1],$nFunc,"BC54","",$logVariSettingAry);
          $totalBal = ($BTCBalance * $baseMultiplier[0][1])-$totalReserved;
          $buyQuantity = $BTCAmount / $baseMultiplier[0][1];
          SuperLog($nFile,"BuyCoins","BaseCurrency is ETH : totalBal: $totalBal | Multiplier : ".$baseMultiplier[0][1],$nFunc,"BC55","RuleID:$ruleIDBuy CoinID:$coinID",$logVariSettingAry);
        }else{
          SuperLog($nFile,"USDT Bal Test : $BTCBalance | $totalReserved ",$nFunc,"BC56","",$logVariSettingAry);
        ...
        if ($totalBal > 15 OR $overrideCoinAlloc == 1) {
          ...
          if ($ruleType == 'Normal'){
            ...
            SuperLog($nFile,"Number of Coin Buys: 1 BuyCounter $userID $coinID | New Buy Counter: ".$buyCounter[$userID."-".$coinID],$nFunc,"BC59","",$logVariSettingAry);
          ...
          $finalBool = True;
        }else{ SuperLog($nFile,"EXIT: $totalBal Less than 20 | $totalBal",$nFunc,"BC64","",$logExitSettingAry);}
      }else{
        if ($limitToCoin != "ALL"){ SuperLog($nFile,"LimitToCoin Found: Continue Rules!",$nFunc,"BC65","",$logFlowSettingAry); continue 2;}
      }

      SuperLog($nFile,"NEXT RULE <BR>",$nFunc,"BC66","",$logFlowSettingAry);
    }//Rule Loop
    echo "</blockquote>";
  }//Coin Loop
  echo "</blockquote>";
...
function buyToreduceLoss($lossCoins){
  ...
  for ($y=0; $y<$lossCoinsSize; $y++){
    ...
    $excludeSpreadBet = 1;
    if ($excludeSpreadBet = 1 and $spreadBetTransactionID <> 0 ){ echo "<BR> ExcludeSpreadBet: EXIT! "; continue;}
