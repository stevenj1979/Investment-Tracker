...
while($completeFlag == False){
  ...
  for($b = 0; $b < $newTrackingSellCoinsSize; $b++) {
    ...
    $lastPrice  = $newTrackingSellCoins[$b][36]; $BTCAmount = $newTrackingSellCoins[$b][37]; $trackingSellID = $newTrackingSellCoins[$b][38];
  ...
  for ($t=0; $t<$buyBackCoinsSize;$t++){
    ...
    if (($priceDifferecePct <=  $buyBackPct) OR ($bullBearStatus == 'BULL')){
      ...
      if ($market1HrChangePct > 0.25){
          $noOfRaisesInPrice = $noOfRaisesInPrice * ($market1HrChangePct/0.25);
      }
