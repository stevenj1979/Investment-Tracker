...
FROM `UserBuyRules` WHERE `UserID` =  $userID";
  ...
  while ($row = mysqli_fetch_assoc($result)){
     ...
     ,$row['Active'],$row['DisableUntil'],$row['BaseCurrency'],$row['NoOfCoinPurchase'],$row['TimetoCancelBuy'],$row['BuyType'],$row['TimeToCancelBuyMins'],$row['BuyPriceMinEnabled'],$row['BuyPriceMin']
      ,$row['LimitToCoin'],$row['AutoBuyCoinEnabled'],$row['AutoBuyPrice'],$row['BuyAmountOverrideEnabled'],$row['BuyAmountOverride']);//35
  }
  $conn->close();
  return $tempAry;
}
...
?>
      ...
      <div class"row">
           ...
           <table>
             <th>&nbsp</th><th>&nbsp</th><th>&nbsp</th><TH>&nbspRuleID</TH><TH>&nbspUserID</TH><TH>&nbspBuyOrdersEnabled</TH><TH>&nbspBuyOrdersTop</TH><TH>&nbspBuyOrdersBtm</TH><TH>&nbspMarketCapEnabled</TH><TH>&nbspMarketCapTop</TH><TH>&nbspMarketCapBtm</TH>
          ...
          for($x = 0; $x < $userSettingsLen; $x++) {
            ...
            echo "<td><a href='AddNewSetting.php?copyRule=".$userSettings[$x][28]."'><span class='fas fa-copy' style='font-size:22px;'></span></a></td>";
            ...
            echo "<td>".$userSettings[$x][48]."</td><td>".$userSettings[$x][49]."</td><tr>";
          }
