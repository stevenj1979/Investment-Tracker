...
function getCoinsfromSQL($userID){
    ...
    $sql = "SELECT `PurchasePrice`,`Year`,`Month`,`Day`,`SellPrice`,`Fee`,`Profit`,`Symbol` FROM `CoinProfitView` WHERE `UserID` = $userID and `Type` = 'Sell' and `Status` = 'Sold'";
    $result = $conn->query($sql);
    //$result = mysqli_query($link4, $query);
	//mysqli_fetch_assoc($result);
    while ($row = mysqli_fetch_assoc($result)){
        $tempAry[] = Array($row['PurchasePrice'],$row['Year'],$row['Month'],$row['Day'],$row['SellPrice'],$row['Fee'],$row['Profit'],$row['Symbol']);
    }
    $conn->close();
    return $tempAry;
}
