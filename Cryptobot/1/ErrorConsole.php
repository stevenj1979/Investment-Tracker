...
function getHeaders(){
  ...
  $sql = "SELECT DISTINCT(`Subject`) as `Subject`  FROM `ErrorLog`  order by `Subject` ";
...
function getsubHeaders(){
  ...
  $sql = "SELECT DISTINCT(`SubTitle`) as `SubTitle` FROM `ErrorLog` order by `SubTitle` ";
  $result = $conn->query($sql);
  //$result = mysqli_query($link4, $query);
  //mysqli_fetch_assoc($result);
  while ($row = mysqli_fetch_assoc($result)){
...
function getConsoleData($console, $userID, $consolsub, $searchtxt){
  ...
  $sql = "SELECT `DateTime`,`Subject`,`Comment`, TimeStampDiff(MINUTE, now(),`DateTime`) As MinsSinceLog, `SubTitle`, `Reference`,`Title`,`LogType` FROM `ErrorLog` WHERE `UserID` = $userID and $sql_option and $sql_option2
...
function main(){
  ...
  echo "<form action='ErrorConsole.php?dropdown=Yes' method='post'>";
