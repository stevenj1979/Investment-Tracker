...
if(isset($_POST['submit'])){
	...
	if(!isset($error)){
      ...
      $sql = "INSERT INTO User (UserName,Password,Email,Active ) VALUES ('$username', '$hashedpassword', '$email', '$activasion')";
			$stmt = $db->prepare($sql);
      //$stmt->bindParam(':username', $username, PDO::PARAM_STR, 12);
      //$stmt->bindParam(':password', $hashedpassword, PDO::PARAM_STR, 12);
      //$stmt->bindParam(':email', $email, PDO::PARAM_STR, 12);
      //$stmt->bindParam(':activation', $activasion, PDO::PARAM_STR, 12);
			//$stmt->execute(Array(':username' => $username,':password' => $hashedpassword,':email' => $email,':activation' => $activasion));
      $stmt->execute();
