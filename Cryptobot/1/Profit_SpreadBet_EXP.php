...
function getCoinsfromSQL($userID){
    ...
    $sql = "SELECT `CoinPrice`*if(`OriginalAmount`=0,`Amount`,`OriginalAmount`) as PurchasePrice,year(`CompletionDate`) as `Year`,month(`CompletionDate`) as `Month`,day(`CompletionDate`) as `Day`,`SellPrice`*if(`OriginalAmount`=0,`Amount`,`OriginalAmount`) as SellPrice
          ...
          if(`BaseCurrency` = 'BTC',((`SellPrice`*if(`OriginalAmount`=0,`Amount`,`OriginalAmount`) )-(`CoinPrice`* if(`OriginalAmount`=0,`Amount`,`OriginalAmount`))-(((`SellPrice`*if(`OriginalAmount`=0,`Amount`,`OriginalAmount`) )/100)*0.28)* 1) ,if(`BaseCurrency` = 'ETH'
            ,((`SellPrice`*if(`OriginalAmount`=0,`Amount`,`OriginalAmount`) )-(`CoinPrice`* if(`OriginalAmount`=0,`Amount`,`OriginalAmount`))-(((`SellPrice`*if(`OriginalAmount`=0,`Amount`,`OriginalAmount`) )/100)*0.28)* 1) ,if(`BaseCurrency` = 'USDT'
             ...
             ,`SpreadBetRuleID`,`SpreadBetTransactionID`,getBTCPrice(84) as `BTCPrice`,getBTCPrice(85) as `ETHPrice`,`BaseCurrency` ,getBTCPrice(83) as `USDTPrice`
    ...
    while ($row = mysqli_fetch_assoc($result)){
        ...
        ,$row['USDProfit'],$row['SpreadBetRuleID'],$row['SpreadBetTransactionID'],$row['BTCPrice'],$row['ETHPrice'],$row['BaseCurrency'],$row['USDTPrice']); //17
    }
    $conn->close();
    return $tempAry;
}
...
function tableHeader($th1,$th2,$th3,$th4,$th5,$th6,$th7,$th8,$th9,$th10,$th11,$th12,$th13){
   Echo "<Table><TH>$th1</TH><TH>$th2</TH><TH>$th3</TH><TH>$th4</TH><TH>$th5</TH><TH>$th6</TH><TH>$th7</TH><TH>$th8</TH><TH>$th9</TH><TH>$th10</TH><TH>$th11</TH><TH>$th12</TH><TH>$th13</TH><TR>";
}

function tableRow($td1,$td2,$td3,$td4,$td5,$td6,$td7,$td8,$td9,$td10,$td11,$td12,$td13){
    Echo "<td>$td1</td><td>$td2</td><td>$td3</td><td>$td4</td><td>$td5</td><td>$td6</td><td>$td7</td><td>$td8</td><td>$td9</td><td>$td10</td><td>$td11</td><td>$td12</td><td>$td13</td><tr>";
}
...
<!--<div class="container">
	...
	<div class="row">
        ...
        tableHeader('Symbol','SB_TransactionID','Original Purchase Price','Sale Price','Fee','Profit BTC','Profit USDT','Profit ETH','Profit USD','Year Sold','Month Sold','Day Sold','% Profit');
        for($x = 0; $x < $arrlength; $x++) {
                    ...
                    $profitBTC = $coins[$x][8]; $profitUSDT = $coins[$x][9]; $profitETH = $coins[$x][10]; $btcPrice = $coins[$x][14];  $ethPrice = $coins[$x][15]; $baseCurrency = $coins[$x][16];
                    $usdtPrice = $coins[$x][17];
