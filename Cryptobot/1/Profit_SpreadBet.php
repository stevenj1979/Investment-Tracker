...
function getCoinsfromSQL($userID){
    ...
    //$sql = "SELECT `UserID`,`OrderNo`,`Symbol`,`Amount`,`Cost`,`TradeDate`,`SellPrice`, `Profit`, `ETHProfit`, `DateSold`, `ID` FROM `Transaction` where `Status` = 'Sold' and `UserID` = $userID order by `DateSold` desc limit 50";
        $sql = "SELECT sum(`CoinPrice`*`Amount`) as PurchasePrice,year(`CompletionDate`) as `Year`,month(`CompletionDate`) as `Month`,day(`CompletionDate`) as `Day`,sum(`SellPrice`*`Amount`) as SellPrice ,sum(((`SellPrice`*`Amount`)/100)*0.28) as Fee, sum((`SellPrice`*`Amount`)-(`CoinPrice`*`Amount`)-(((`SellPrice`*`Amount`)/100)*0.28)) as Profit,'Symbol',if(`BaseCurrency` = 'BTC',sum((`SellPrice`*`Amount`)-(`CoinPrice`*`Amount`)-(((`SellPrice`*`Amount`)/100)*0.28)),0) as BTCProfit, if(`BaseCurrency` = 'USDT',sum((`SellPrice`*`Amount`)-(`CoinPrice`*`Amount`)-(((`SellPrice`*`Amount`)/100)*0.28)),0) as USDTProfit
