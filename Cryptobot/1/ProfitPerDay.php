...
function getCoinsfromSQL($userID){
    ...
    $sql = "SELECT sum(`PurchasePrice`)as PurchasePrice,`Year`,`Month`,`Day`,Sum(`SellPrice`) as SellPrice,Sum(`Fee`) as Fee ,Sum(`Profit`) as Profit
      FROM `CoinProfitView`
    ...
    while ($row = mysqli_fetch_assoc($result)){
        $tempAry[] = Array($row['PurchasePrice'],$row['Year'],$row['Month'],$row['Day'],$row['SellPrice'],$row['Fee'],$row['Profit']);
    }
    $conn->close();
    return $tempAry;
}
