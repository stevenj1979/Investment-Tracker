...
function getCoinsfromSQL($userID){
    ...
    $sql = "SELECT sum(`CoinPrice`* if(`OriginalAmount`=0,`Amount`,`OriginalAmount`)) as PurchasePrice,max(year(`CompletionDate`)) as `Year`,Max(month(`CompletionDate`)) as `Month`,Max(day(`CompletionDate`)) as `Day`,sum(`SellPrice`*if(`OriginalAmount`=0,`Amount`,`OriginalAmount`)) as SellPrice
              ,sum(((`SellPrice`*if(`OriginalAmount`=0,`Amount`,`OriginalAmount`))/100)*0.28) as Fee, sum((`SellPrice`*if(`OriginalAmount`=0,`Amount`,`OriginalAmount`))-(`CoinPrice`* if(`OriginalAmount`=0,`Amount`,`OriginalAmount`))-(((`SellPrice`*if(`OriginalAmount`=0,`Amount`,`OriginalAmount`))/100)*0.28)) as Profit,'Symbol',if(`BaseCurrency` = 'BTC'
                ,sum((`SellPrice`*if(`OriginalAmount`=0,`Amount`,`OriginalAmount`))-(`CoinPrice`* if(`OriginalAmount`=0,`Amount`,`OriginalAmount`))-(((`SellPrice`*if(`OriginalAmount`=0,`Amount`,`OriginalAmount`))/100)*0.28)),0) as BTCProfit, if(`BaseCurrency` = 'USDT'
                  ,sum((`SellPrice`*if(`OriginalAmount`=0,`Amount`,`OriginalAmount`))-(`CoinPrice`* if(`OriginalAmount`=0,`Amount`,`OriginalAmount`))-(((`SellPrice`*if(`OriginalAmount`=0,`Amount`,`OriginalAmount`))/100)*0.28)),0) as USDTProfit
              ,if(`BaseCurrency` = 'ETH',sum((`SellPrice`*if(`OriginalAmount`=0,`Amount`,`OriginalAmount`))-(`CoinPrice`* if(`OriginalAmount`=0,`Amount`,`OriginalAmount`))-(((`SellPrice`*if(`OriginalAmount`=0,`Amount`,`OriginalAmount`))/100)*0.28)),0) as ETHProfit,
              if(`BaseCurrency` = 'BTC',sum((`SellPrice`*if(`OriginalAmount`=0,`Amount`,`OriginalAmount`))-(`CoinPrice`* if(`OriginalAmount`=0,`Amount`,`OriginalAmount`))-(((`SellPrice`*if(`OriginalAmount`=0,`Amount`,`OriginalAmount`))/100)*0.28)* getBTCPrice(84)) ,if(`BaseCurrency` = 'ETH'
                ,sum((`SellPrice`*if(`OriginalAmount`=0,`Amount`,`OriginalAmount`))-(`CoinPrice`* if(`OriginalAmount`=0,`Amount`,`OriginalAmount`))-(((`SellPrice`*if(`OriginalAmount`=0,`Amount`,`OriginalAmount`))/100)*0.28)* getBTCPrice(85)) ,if(`BaseCurrency` = 'USDT'
                 ...
                 ,`SpreadBetRuleID`,`SpreadBetTransactionID`,`BuyBackTransactionID` FROM `View15_OpenTransactions`
  ...
  //echo "<BR> $sql";
    while ($row = mysqli_fetch_assoc($result)){
        ...
        ,$row['USDProfit'],$row['SpreadBetRuleID'],$row['SpreadBetTransactionID'],$row['BuyBackTransactionID']);
    }
    $conn->close();
    return $tempAry;
}
...
<!--<div class="container">
	...
	<div class="row">
        ...
        for($x = 0; $x < $arrlength; $x++) {
                    ...
                    tableRow($BuyBackTransID,$purchasePrice,$sellPrice,$fee,$profitBTC, $profitUSDT, $profitETH, $profitUSD,$sellYear,$sellMonth,$sellDay);
