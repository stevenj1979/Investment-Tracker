...
function getTrackingSellCoinsLoc($userID){
  ...
  if ($conn->connect_error) {
    ...
    ,`SpreadBetRuleName`,`Profit`,`PurchasePrice`,`LivePrice`
    FROM `SellCoinsSpreadGroupView` WHERE `UserID` = $userID ORDER BY `ProfitPct` Desc";
  $result = $conn->query($sql);
    //print_r($sql);
  //$result = mysqli_query($link4, $query);
//mysqli_fetch_assoc($result);`PctChange1Hr`, `PctChange24Hr`, `PctChange7D`
  while ($row = mysqli_fetch_assoc($result)){
      ...
      ,$row['ToMerge'],$row['LowPricePurchaseEnabled'],$row['PurchaseLimit'],$row['PctToPurchase'],$row['BTCBuyAmount'],$row['NoOfPurchases'],$row['Name'],$row['Image'],$row['MaxCoinMerges'],$row['NoOfCoinSwapsThisWeek'],$row['SpreadBetRuleName']
    ,$row['Profit'],$row['PurchasePrice'],$row['LivePrice']); //57
  }
  $conn->close();
  return $tempAry;
}
