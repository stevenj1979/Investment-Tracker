...
function WritetoRule($coinD, $ruleID, $highPrice, $lowPrice, $buyAmount, $enable, $type, $sellRuleID,$numOfRisesInPrice, $minsToCancel){
  ...
  $sql = "call UpdateRulesforCoinMode($coinD,$ruleID,$newHighPrice,$newLowPrice,$buyAmount,$enable,$type,$sellRuleID,$numOfRisesInPrice,$minsToCancel);";
...
function sendCoinModeEmail($to, $symbol, $hr1Price, $hr24Price, $d7Price, $subject, $user, $mode, $pctToBuy,$numOfRisesInPrice,$new6MonthHighPrice,$new6MonthLowPrice,$livePrice,$newProjectedMaxPrice,$newProjectedMinPrice,$buyAmount){
        ...
        if ($buyAmount >= $minBuyAmount){
          ...
          if ($modeID <> 1){
            ...
            if ($coinModeEmailsEnabled == 1){
              sendCoinModeEmail($email,$symbol,$Hr1AveragePrice,$pctInc24Hours,$pctInc7Day, "$symbol Buy Mode Activated",$userName, "Buy Mode",$pctToBuy,$numOfRisesInPrice,$new6MonthHighPrice,$new6MonthLowPrice,$livePrice,$newProjectedMaxPrice,$newProjectedMinPrice,$buyAmount);
            }
          }
        ...
        if ($t1 == True and $t2 == True and $t3 == True){
          ...
          for ($i=0; $i<$secondarySellRulesSize; $i++){
            if ($secondarySellRulesAry[$i] <> ""){
              WritetoRule($coinID,$ruleID,$newProjectedMaxPrice,$newLowPrice, 0, 1, 2,$secondarySellRulesAry[$i],$numOfRisesInPrice,$minsToCancelBuy);
            }
          }
          WritetoRule($coinID,$ruleID,$newProjectedMaxPrice,$newLowPrice, 0, 1, 2,$ruleIDSell,$numOfRisesInPrice,$minsToCancelBuy);
          if ($modeID <> 2 ){
            logToSQL("CoinModeSell","Change Coin mode to 2 for: $symbol ($coinID) | $livePrice", $userID, 1);
            if ($coinModeEmailsEnabled == 1){
              sendCoinModeEmail($email,$symbol,$Hr1AveragePrice,$pctInc24Hours,$pctInc7Day, "$symbol Sell Mode Activated",$userName, "Sell Mode",$pctToBuy,$numOfRisesInPrice,$newProjectedMaxPrice,$newProjectedMinPrice,$livePrice,$newProjectedMaxPrice,$newProjectedMinPrice,0);
            }
          }
          return True;
        }else{
    ...
    function isFlatMode($coinAry, $forceFlat){
      ...
      //for ($i=0; $i<$coinArySize; $i++){
          ...
          if ($t1 == True and $t2 == True and $t3 == True or $forceFlat = 1){
            ...
            for ($i=0; $i<$secondarySellRulesSize; $i++){
              if ($secondarySellRulesAry[$i] <> ""){
                WritetoRule($coinID,$ruleID,0,0, 0, 0, 3, $secondarySellRulesAry[$i],0,$minsToCancelBuy);
              }
            }
            echo "<BR> Activate FLAT MODE";
            WritetoRule($coinID,$ruleID,0,0, 0, 0, 3,$ruleIDSell,0,$minsToCancelBuy);
      ...
      function getCoins(){
        ...
        ,`D7Top`,`D7Btm`,`SecondarySellRules`,`CoinModeEmails`,`Email`,`UserName`, `Symbol`,`CoinModeEmailsSell`,`CoinModeMinsToCancelBuy` FROM `CoinModePricesView`";
      ...
      ,$row['Symbol'],$row['CoinModeEmailsSell'],$row['CoinModeMinsToCancelBuy']); //21
        }
        $conn->close();
        return $tempAry;
      }
