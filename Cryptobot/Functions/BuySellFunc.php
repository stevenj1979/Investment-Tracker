  ...
  echoText("Tracking Buy Count 4 <BR>",$echoTestText);
    ...
    if ($readyToBuy == 1){
      ...
      if ($type == 'SavingsBuy'){
        ...
        newLogToSQL("TrackingCoin","trackingCoinReadyToBuy($liveCoinPrice,$timeToCancelBuyMins,$type,$originalPrice,$newTrackingCoinID,$noOfRisesInPrice,$pctProfit,$minsFromDate,$lastPrice,$risesInPrice,$trackingID,$quickBuyCount,$market1HrChangePct)$coinID|$overrideCoinAlloc|".$coinAllocation." | $type | $coinMode;",$userID,1,"TrackingSuccess","TrackingCoinID:$newTrackingCoinID");
        if (!Empty($KEK)){ $APISecret = Decrypt($KEK,$newTrackingCoins[$a][19]);}
        ...
        if ($type == 'buyToreduceLoss' AND $SBRuleID <> 0 AND $SBTransID <> 0){ $newTrackingType = 'SpreadBuy';}
        ...
        if ($checkBuy == 1){
          SuperLog($nFile,"buyCoins($APIKey, $APISecret,$symbol, $Email, $userID, $date, $baseCurrency,$SendEmail,$BuyCoin,$ogBTCAmount, $ruleIDBuy,$UserName,$coinID,$CoinSellOffsetPct,$CoinSellOffsetEnabled,$buyType,$timeToCancelBuyMins,$SellRuleFixed, $liveCoinPrice, $noOfPurchases+1);",$nFunc,"TC1","",$logEventsSettingAry,'Events');
