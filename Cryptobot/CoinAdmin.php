...
function updateSQLactive($userID){
  $conn = getSQLConn(rand(1,3));
    ...
    $conn->close();
    newLogToSQL("updateSQLactive",$sql,3,1,0,"SQL","UserID:$userID");
}
...
function updateFixSellRule($newFixRule, $transactionID){
    ...
    $conn->close();
    newLogToSQL("updateFixSellRule",$sql,3,1,0,"SQL","TransactionID:$transactionID");
}
...
function writePrice($coinID, $price, $month, $year, $minPrice){
  ...
  $conn->close();
  newLogToSQL("writePrice",$sql,3,1,0,"SQL CALL","CoinID:$coinID");
}
...
function checkRuleIsOpen($id){
  ...
  $conn->close();
  newLogToSQL("checkRuleIsOpen",$sql,3,1,0,"SQL CALL","RuleID:$id");
}
...
function clearOrphanedRules($sellRule, $coinID, $userID){
  ...
  $conn->close();
  newLogToSQL("clearOrphanedRules",$sql,3,1,0,"SQL CALL","SellRuleID:$sellRule UserID:$userID");
}
...
function writeHigh($coinID, $price){
  ...
  $conn->close();
  newLogToSQL("writeHigh",$sql,3,1,0,"SQL CALL","CoinID:$coinID");
}
...
function writeLow($coinID, $price){
  ...
  $conn->close();
  newLogToSQL("writeLow",$sql,3,1,0,"SQL CALL","CoinID:$coinID");
}
...
function updateCoinPct($coinID,$buyRuleID, $mode){
  ...
  $conn->close();
  newLogToSQL("updateCoinPct",$sql,3,1,0,"SQL CALL","CoinID:$coinID BuyRuleID:$buyRuleID");
}
...
function updateSpreadPct($coinID,$spreadBetRuleID, $mode){
  ...
  $conn->close();
  newLogToSQL("updateSpreadPct",$sql,3,1,0,"SQL","CoinID:$coinID SBRuleID:$spreadBetRuleID");
}
...
function updateSpreadBetCoinHistory(){
  ...
  $conn->close();
  newLogToSQL("updateSpreadBetCoinHistory",$sql,3,1,0,"SQL CALL","");
}
